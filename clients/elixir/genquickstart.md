...
```elixir
# TODO: publish corndogs to Hex, then add it (and the transport lib) to deps:
def deps do
  [
    {:corndogs, "~> 0.0.0"},
...
resp = Csilgen.Generated.CorndogsClient.submit_task(client, %Csilgen.Generated.SubmitTaskRequest{queue: "example", current_state: "example", auto_target_state: "example", timeout: 0, payload: <<>>, priority: 0})
IO.inspect(resp)
```
...
req = %Csilgen.Generated.SubmitTaskRequest{queue: "example", current_state: "example", auto_target_state: "example", timeout: 0, payload: <<>>, priority: 0}
payload = Csilgen.Generated.SubmitTaskRequest.to_cbor(req)
...
case UdpDatagramCarrier.recv_datagram(carrier) do
  {:ok, bytes, _carrier} ->
    ...
    resp = Csilgen.Generated.SubmitTaskResponse.from_cbor(dg.payload)
