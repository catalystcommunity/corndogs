...
```sh
npm install corndogs csilgen-transport
```
...
import { AsyncApiClient, type AsyncServiceTransport } from "corndogs";
...
async function main() {
  ...
  const resp = await client.corndogs.submitTask({ queue: "example", currentState: "example", autoTargetState: "example", timeout: 0, payload: new Uint8Array(), priority: 0 });
  console.log(resp);
}
...
import { toSubmitTaskRequestCbor, fromSubmitTaskResponseCbor } from "corndogs";
import type { SubmitTaskRequest, SubmitTaskResponse } from "corndogs";
...
async function main() {
  ...
  const req: SubmitTaskRequest = { queue: "example", currentState: "example", autoTargetState: "example", timeout: 0, payload: new Uint8Array(), priority: 0 };
  await carrier.sendDatagram(new Datagram(OP_ORD, 0, toSubmitTaskRequestCbor(req)).encode());
  ...
  if (inbound !== null) {
    ...
    const resp: SubmitTaskResponse = fromSubmitTaskResponseCbor(dg.payload);
