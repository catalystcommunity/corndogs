# github.com/CatalystCommunity/corndogs/clients/corndogs
...
func main() {
	client := api.NewCorndogsClient(&HTTPRpcCarrier{BaseURL: "http://localhost:5080"})
	resp, err := client.SubmitTask(context.Background(), api.SubmitTaskRequest{Queue: "example", CurrentState: "example", AutoTargetState: "example", Timeout: 0, Payload: nil, Priority: 0})
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", resp)
...
func runDatagrams(carrier transport.DatagramCarrier) error {
	req := api.SubmitTaskRequest{Queue: "example", CurrentState: "example", AutoTargetState: "example", Timeout: 0, Payload: nil, Priority: 0}
	// seq 0 marks an unsequenced datagram.
	datagram, err := transport.NewDatagram(opOrd, 0, api.EncodeSubmitTaskRequest(req)).Encode()
	if err != nil {
		return err
	}
	...
	if inbound != nil {
		...
		resp, err := api.DecodeSubmitTaskResponse(dg.Payload)
		if err != nil {
			return err
		}
