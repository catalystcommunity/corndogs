...
.package(path: "./corndogs"),
.package(path: "../csilgen/transports/swift"),
```

and list both products in your target's `dependencies`: `"Corndogs"` and
...
let client = CorndogsClient(transport: HttpRpcCarrier(baseURL: "http://localhost:5080"))
let result = try client.submitTask(SubmitTaskRequest(queue: "example", currentState: "example", autoTargetState: "example", timeout: 0, payload: [], priority: 0))
print(result)
```
...
func main() throws {
    ...
    // datagram.
    let req: SubmitTaskRequest = SubmitTaskRequest(queue: "example", currentState: "example", autoTargetState: "example", timeout: 0, payload: [], priority: 0)
    ...
    if let inbound = try carrier.recvDatagram() {
        ...
        let resp = try SubmitTaskResponse.fromCbor(dg.payload)
