...
pub fn main() !void {
    ...
    const svc = client.CorndogsClient.init(carrier.transport());
...
pub fn main() !void {
    ...
    const req = types.SubmitTaskRequest{ .queue = "example", .current_state = "example", .auto_target_state = "example", .timeout = 0, .payload = undefined, .priority = 0 };
    const payload = try codec_gen.encode_SubmitTaskRequest(alloc, &req);
    ...
    if (try carrier.recv(arena.allocator())) |inbound| {
        ...
        var resp: types.SubmitTaskResponse = undefined;
        try codec_gen.decode_SubmitTaskResponse(arena.allocator(), dg.payload, &resp);
        std.debug.print("late response\n", .{});
    }
}
```
