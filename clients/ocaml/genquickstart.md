...
(executable
 (name main)
 (libraries corndogs csilgen-transport unix))
```

<!-- TODO: `csilgen-transport` is not yet published — vendor it or pin a git
source until it lands in an opam repository, then `opam install corndogs`. -->
...
let () =
  ...
  let req : Types.submit_task_request = { queue = "example"; current_state = "example"; auto_target_state = "example"; timeout = 0L; payload = (Bytes.of_string ""); priority = 0L } in
  match Client.Corndogs_service.submit_task client req with
...
let () =
  ...
  let req : Types.submit_task_request = { queue = "example"; current_state = "example"; auto_target_state = "example"; timeout = 0L; payload = (Bytes.of_string ""); priority = 0L } in
  let dg = Datagrams.new_datagram op_ord 0L (Codec.encode_submit_task_request_bytes req) in
  ...
  | Ok (Some bytes) -> (
    ...
    | Ok (dg : Datagrams.datagram) ->
      let resp = Codec.decode_submit_task_response_bytes dg.payload in
      ignore resp;
