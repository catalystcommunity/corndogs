...
```sh
pip install corndogs csilgen-transport
```
...
def main() -> None:
    client = CorndogsClient(HttpRpcCarrier("http://localhost:5080"))
    resp = client.submit_task(SubmitTaskRequest(queue="example", current_state="example", auto_target_state="example", timeout=0, payload=b"", priority=0))
    print(resp)
...
from corndogs import SubmitTaskRequest, SubmitTaskResponse
...
def main() -> None:
    ...
    req = SubmitTaskRequest(queue="example", current_state="example", auto_target_state="example", timeout=0, payload=b"", priority=0)
    ...
    if inbound is not None:
        ...
        resp = SubmitTaskResponse.from_cbor(dg.payload)
