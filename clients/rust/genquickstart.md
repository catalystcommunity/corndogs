...
fn main() {
    let client = CorndogsClient::new(HttpRpcCarrier::new("http://localhost:5080"));
    let resp = client.submit_task(SubmitTaskRequest { queue: "example".to_string(), current_state: "example".to_string(), auto_target_state: "example".to_string(), timeout: 0, payload: Vec::new(), priority: 0 });
    match resp {
...
fn run_datagrams(carrier: &mut impl DatagramCarrier) -> Result<(), Box<dyn std::error::Error>> {
    let req = SubmitTaskRequest { queue: "example".to_string(), current_state: "example".to_string(), auto_target_state: "example".to_string(), timeout: 0, payload: Vec::new(), priority: 0 };
    // seq 0 marks an unsequenced datagram.
    carrier.send_datagram(&Datagram::new(OP_ORD, 0, encode_submit_task_request(&req)).encode()?)?;

    if let Some(inbound) = carrier.recv_datagram()? {
        let dg = Datagram::decode(&inbound)?;
        let resp = decode_submit_task_response(&dg.payload)?;
