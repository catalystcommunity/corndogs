...
fun main() {
    val client = CorndogsClient(HttpRpcTransport("http://localhost:5080"))
    val resp = client.submitTask(SubmitTaskRequest(queue = "example", currentState = "example", autoTargetState = "example", timeout = 0L, payload = ByteArray(0), priority = 0L))
    println(resp)
}
```
...
fun main() {
    ...
    val req: SubmitTaskRequest = SubmitTaskRequest(queue = "example", currentState = "example", autoTargetState = "example", timeout = 0L, payload = ByteArray(0), priority = 0L)
    ...
    if (inbound != null) {
        ...
        val resp: SubmitTaskResponse = submitTaskResponseFromCbor(dg.payload)
