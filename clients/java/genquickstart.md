...
public final class RpcExample {
    public static void main(String[] args) {
        CorndogsClient client = new CorndogsClient(new HttpRpcCarrier("http://localhost:5080"));
        SubmitTaskResponse resp = client.submitTask(new SubmitTaskRequest("example", "example", "example", 0L, new byte[0], 0L));
...
public final class DatagramsExample {
    ...
    static void sendRequest(DatagramCarrier carrier, SubmitTaskRequest request) throws IOException {
        carrier.sendDatagram(Datagrams.Datagram.of(OP_ORD, 0, CsilCbor.encodeSubmitTaskRequest(request)).encode());
    }
    ...
    static SubmitTaskResponse recvResponse(DatagramCarrier carrier) throws IOException {
        byte[] inbound = carrier.recvDatagram();
        if (inbound == null) {
            return null;
        }
        Datagrams.Datagram dg = Datagrams.Datagram.decode(inbound);
        return CsilCbor.decodeSubmitTaskResponse(dg.payload());
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        sendRequest(carrier, new SubmitTaskRequest("example", "example", "example", 0L, new byte[0], 0L));
        // No synchronous reply: a response datagram may arrive later, or never.
        SubmitTaskResponse resp = recvResponse(carrier);
        System.out.println(resp);
    }
}
```
