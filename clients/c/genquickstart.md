...
int main(void) {
    ...
    if (csil_encode_SubmitTaskRequest(&req, &payload, &payload_len)) { close(fd); return 1; }
    csil_datagram dg;
    ...
    if (carrier.recv_datagram(carrier.userdata, &inb, &inn) == CSIL_OK && inb) {
        ...
        if (csil_datagram_decode(inb, inn, &in_dg) == 0) {
            SubmitTaskResponse resp;
            CsilCodecArena *owner = NULL;
            if (csil_decode_SubmitTaskResponse(in_dg.payload, in_dg.payload_len, &resp, &owner) == 0) {
